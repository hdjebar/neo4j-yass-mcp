...
def find_available_port(
    host: str, preferred_ports: list[int], fallback_range: tuple = (8000, 9000)
) -> int | None:
    """
    ...
    logger.warning(
        f"All preferred ports are in use. Scanning range {fallback_range[0]}-{fallback_range[1]}"
    )
...
def get_preferred_ports_from_env(
    env_var: str = "PREFERRED_PORTS_MCP", default: str = "8000 8001 8002"
) -> list[int]:
    """
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"Invalid port configuration in {env_var}, using defaults"
        )
